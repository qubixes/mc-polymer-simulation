...
int SimStateInit(SimState* ss, SimProperties* sp){
	...
	for(int iDev=0; iDev<sp->nDevices; iDev++){
		curState = ss+iDev;
		curState->lattice = (uint*) malloc(sp->latSize*sizeof(uint));
		curState->slLattice = (uint*) malloc(sp->slSize*sizeof(uint));
		curState->labLattice = (uint*) malloc(sp->labSize*sizeof(uint));
		printf("labSize = %i\n", sp->labSize);
		curState->nPol=0;
...
void ClearCPULattice(SimState* ss, SimProperties* sp){
	for(int i=0; i<sp->latSize; i++) ss->lattice[i]   =0;
	for(int i=0; i<sp->slSize;  i++) ss->slLattice[i] =0;
	for(int i=0; i<sp->labSize; i++) ss->labLattice[i]=0;
}
...
void SetBoxDimension(SimProperties* sp, int LT, int LU, int LV){
	...
	sp->latSize = sp->L/MSPIN+1;
	sp->slSize = sp->L/SL_MSPIN+1;
	sp->labSize = sp->L/LAB_MSPIN+1;
	sp->gpuLatSize = sp->L;
		...
		for(int du=1; du<LU; du++){
			...
			for(int dv=2; dv<LV; dv+=2){
				...
				if(dt*du*dv*2 > polSize && nt*nu*nv >= nPol){
					double sl=(polSize-nt*nu*nv)/(double)polSize;
...
void GetBestPolymerPartitioning(SimProperties* sp, SimState* ss, int polLength, double density){
	...
	if(polLength<100){
		...
		nPolAdded=0;
		for(uint t=0; t<nt*dt && nPolAdded<nPol; t += dt){
			for(uint u=0; u<nu*du  && nPolAdded<nPol; u += du){
				for(uint v=0; v<nv*dv && nPolAdded<nPol; v += dv){
					for(int iDev=0; iDev<sp->nDevices; iDev++)
...
// 			printf("(%u %u %u)\n", t,u,v);
// 			printf("iPol = %i\n", iPol);
			SetBondVecs(ss+iDev, t,u,v,pol->bonds,pol->length,pol->label);
// 			exit(0);
		}
	}
...
void CreateGPULattice(SimState* ss, SimProperties* sp){
	for(SimState* curState=ss; curState<ss+sp->nDevices; curState++)
		curState->gpuLattice = (uint*) malloc(sizeof(uint)*sp->gpuLatSize); 
	CopyCPUToGPULattice();
}
