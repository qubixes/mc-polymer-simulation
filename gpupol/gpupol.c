...
// 	GPULibInit(&sp, ss, devices, &gpuContext);
	...
	for(;sp.curT < sp.tMax; sp.curT += sp.writeInterval){
		...
		fprintf(pFile, "%s[%.1lf%%] Time elapsed = %.2f s at %.2f MF/s", cleanStr, 100*(1-(sp.tMax-sp.curT-sp.writeInterval)/totStep), totT, sp.writeInterval*sp.nwg*sp.nSteps*sp.ws*sp.nDevices/totT/1000000.0);
		fflush(pFile);
