...
if [ $# -lt 2 ]; then
	echo "Need two arguments: directory and HP strength"
	exit 192;
fi

if [ $# -ge 3 -a "$3" == "TOPO" ]; then
	EXEC=$BIN_DIR/denspol_ring_hp_topo
	TOPO="TRUE"
else
	EXEC=$BIN_DIR/denspol_ring_hp
	TOPO="FALSE"
fi
...
if [ $TOPO == "TRUE" ]; then
	BASE_DEST_DIR="$BASE_DIR/rec_l${START_LENGTH}_hp${HP_STRENGTH}_t${TIME}_TOPO"
else
	BASE_DEST_DIR="$BASE_DIR/rec_l${START_LENGTH}_hp${HP_STRENGTH}_t${TIME}"
fi

FIRST_DIR=$BASE_DEST_DIR
mkdir -p $FIRST_DIR
SRC_FILE=$DIR/`get_last_tfile $DIR`
$BIN_DIR/contact_map $SRC_FILE $FIRST_DIR/contact_map.dat || exit $?
...
if [ $LAST_TFILE == "NOT_FOUND" -o $LAST_TFILE == "t=0_dev=0.res" ]; then
	echo "$EXEC $SEED $FIRST_DIR $DENSITY $TIME $INTERVAL $START_LENGTH $START_L ./denspol/ee_topo.dat $BEND_ENERGY 0 $SHUFFLE $FIRST_DIR/contact_map.dat $HP_STRENGTH"
	$EXEC $SEED $FIRST_DIR $DENSITY $TIME $INTERVAL $START_LENGTH $START_L ./denspol/ee_topo.dat $BEND_ENERGY 0 $SHUFFLE $FIRST_DIR/contact_map.dat $HP_STRENGTH || exit $?
...
while [ $CUR_LENGTH -lt $LENGTH ]; do
	...
	if [ $CUR_TFILE == "NOT_FOUND" ]; then
		...
		$EXEC $SEED $SECOND_DIR $DENSITY $TIME $INTERVAL $CUR_LENGTH $CUR_L ./denspol/ee_topo.dat $BEND_ENERGY 1 0 $SECOND_DIR/contact_map.dat $HP_STRENGTH
...
# echo "$BIN_DIR/denspol_ring_hp $SEED $FIRST_DIR $DENSITY $TIME $INTERVAL $START_LENGTH $START_L ./denspol/ee_topo.dat $BEND_ENERGY 0 $SHUFFLE $FIRST_DIR/contact_map.dat $HP_STRENGTH"
# exit 0
